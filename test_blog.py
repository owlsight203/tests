...
def test_content_form(page: Page):
	page.goto("127.0.0.1:8080/post/14")

	content_box = page.locator("textarea[name='content']").first
	content_box.fill("Bình luận từ Playwright")

	gemini_radio = page.locator("input#ai_gemini").first
	gemini_radio.click()

	expect(content_box).to_have_value("Bình luận từ Playwright")
	expect(gemini_radio).to_be_checked()
